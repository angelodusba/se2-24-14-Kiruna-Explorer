        ...
        return {
            createConnection: jest.fn().mockImplementation((id: any) => {
                if (id === -1) {
                    ...
                    return Promise.reject(new ConnectionAlreadyExistsError(1,2,"mocking"));
                }
                return Promise.resolve({ id });
            }),
            getConnections: jest.fn().mockImplementation(() => {
                return Promise.resolve([{ document_id_1: 1, document_id_2: 2, connection_name: "direct_conn" }]);
            }),
...
describe("ConnectionController", () => {
    ...
    describe("createConnections", () => {
        test("should create a new connection", async () => {
            ...
            let connections = [{ connected_document_id: 2, connection_name: "direct_conn" },
                                { connected_document_id: 3, connection_name: "collateral_conn" },
                                 { connected_document_id: 4, connection_name: "indirect_conn" }];
            //mocking the case using starting_document_id = -1
            let result = connectionController.createConnections(starting_document_id, connections);
    ...
    describe("getConnections", () => {
        test("should return all connections", async () => {
            ...
            expect(result).resolves.toEqual([{ document_id_1: 1, document_id_2: 2, connection_name: "direct_conn" }]);
        });
    });
