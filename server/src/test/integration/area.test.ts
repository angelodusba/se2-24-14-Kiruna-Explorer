...
beforeAll(async () => {
  ...
  // Populate DB with municipality area
  await db.query(`INSERT INTO areas (name, location) VALUES ($1, 
  ...
  test("GET /kirunaexplorer/areas - success", async () => {
    await request(app)
      ...
      .expect((res) => {
        ...
        expect(res.body.every((area: any) => area.id > 0)).toBe(true);
        expect(res.body.every((area: any) => area.name === "area1")).toBe(true);
        expect(res.body.every((area: any) => area.location.length > 1 && area.location.every((point: any) => typeof point.lat === "number" && typeof point.lng === "number"))).toBe(true);
      });
  });
});
