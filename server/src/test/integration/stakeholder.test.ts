...
describe("GET kirunaexplorer/stakeholders", () => {
  test("GET /kirunaexplorer/stakeholders - success", async () => {
    await request(app)
      .get(`${routePath}/stakeholders`)
      .expect(200)
      .expect((res) => {
        expect(res.body.length).toBe(2);
        expect(res.body.some((stakeholder: any) => stakeholder.name === "stakeholder1")).toBe(true);
        expect(res.body.some((stakeholder: any) => stakeholder.name === "stakeholder2")).toBe(true);
        expect(res.body.some((stakeholder: any) => stakeholder.name === "stakeholder3")).toBe(false);
      });
  });
});

describe("POST kirunaexplorer/stakeholders", () => {
  test("POST /kirunaexplorer/stakeholders - success", async () => {
    await request(app)
      .post(`${routePath}/stakeholders`)
      .set("Cookie", urbanPlannerCookie)
      .send({ name: "stakeholder3" })
      .expect(200);
    ...
    await request(app)
      ...
      .expect((res) => {
        ...
        expect(res.body.some((stakeholder: any) => stakeholder.name === "stakeholder3")).toBe(true);
      });
  });

  test("POST /kirunaexplorer/stakeholders - Stakeholder already exists", async () => {
    await request(app)
      .post(`${routePath}/stakeholders`)
      .set("Cookie", urbanPlannerCookie)
      .send({ name: "stakeholder1" })
      .expect(409)
      .expect((res) => {
        expect(res.body.error).toBe("A stakeholder with this name already exists");
      });
  });

  test("POST /kirunaexplorer/stakeholders - Not logged in", async () => {
    await request(app)
      .post(`${routePath}/stakeholders`)
      .send({ name: "stakeholder3" })
      .expect(401)
